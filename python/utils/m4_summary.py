...
def group_values(values, groups, group_name):
    x = [v[~np.isnan(v)] for v in values]
    #print(f'[group_values] x : {x}')
   #x2 = x[groups == group_name]
    return x# [v[~np.isnan(v)] for v in values[groups == group_name]]
...
class M4Summary:
    ...
    def evaluate(self):
        ...
        for group_name in M4Meta.seasonal_patterns:
            ...
            print(f'[] Grouping based on group_name {group_name}')
